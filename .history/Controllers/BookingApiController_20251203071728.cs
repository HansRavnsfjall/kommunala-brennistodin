...
#pragma warning restore CS0618
        ...
        [HttpGet("ping")]
        [AllowAnonymous]
        public IActionResult Ping() => Ok(new { ok = true, msg = "booking api alive", version = "v3.2-email-firm-range-yearguard-publicics" });
            ...
            if (settings != null)
            {
                ...
                if (bl != null)
                {
                    foreach (var item in bl)
                    {
                        ...
                        if (globalTo < globalFrom) (globalFrom, globalTo) = (globalTo, globalFrom); // swap if inverted
...
#pragma warning restore CS0618
        ...
        // ICS feed (Approved only, full info): /umbraco/api/bookingapi/feed.ics
        [HttpGet("feed.ics")]
        [AllowAnonymous]
...
#pragma warning restore CS0618
        ...
        // ICS feed (Approved only, NO personal info):
...
#pragma warning restore CS0618
            ...
            // Build base URL for backoffice link
            var scheme = Request?.Scheme ?? "https";
            var host   = Request?.Host.ToString() ?? "www.kob.fo";
            ...
            foreach (var b in bookings)
            {
                ...
                var start = dt.Value.ToString("yyyyMMdd");
                var backofficeUrl = $"{baseUrl}/umbraco#/content/content/edit/{b.Id}";

                // Generic, non-personal summary
                var summary = $"Vitjan – {label}";

                // Description only contains a link back to Umbraco
                var description = $"Sí booking í Umbraco: {backofficeUrl}";
