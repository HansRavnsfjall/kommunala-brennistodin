...
#pragma warning restore CS0618
        ...
        public IActionResult Ping() => Ok(new { ok = true, msg = "booking api alive", version = "v3.4-ignore-bad-ranges" });
...
#pragma warning restore CS0618
            ...
            // Calendar window we care about
            var start = new DateOnly(year, month, 1);
            var end   = start.AddMonths(months); // end-exclusive
            ...
            // blocked dates from settings (supports date ranges, intersected with [start,end))
            if (settings != null)
            {
                ...
                if (bl != null)
                {
                    foreach (var item in bl)
                    {
                        ...
                        // treat uninitialized date (year < 1900) as empty
                        if (fromDt == null || fromDt.Value.Year < 1900)
                            continue;

                        var toDt = item?.Content?.Value<DateTime?>("dateTo");
                        if (toDt == null || toDt.Value.Year < 1900)
                            toDt = fromDt;

                        var globalFrom = DateOnly.FromDateTime(fromDt.Value.Date);
                        var globalTo   = DateOnly.FromDateTime(toDt.Value.Date);
                        // if editor misconfigures (to < from), ignore this blockedDates item
                        if (globalTo < globalFrom)
                            continue;

                        // no overlap with calendar window → skip
                        if (globalTo < start || globalFrom >= end)
                            continue;

                        // clamp to calendar window
                        var rangeFrom = globalFrom < start ? start : globalFrom;
                        var rangeTo   = globalTo >= end ? end.AddDays(-1) : globalTo;
                        ...
                        for (var dIt = rangeFrom; dIt <= rangeTo; dIt = dIt.AddDays(1))
                        {
                            if (debug && debugInfo.Count < 500)
                            {
                                debugInfo.Add(new
                                {
            ...
            if (bookingsRoot != null)
            {
                foreach (var r in bookingsRoot.Children().Where(x => x.ContentType.Alias == "bookingRequest"))
                {
                    ...
                    if (dOnly < start || dOnly >= end) continue; // outside calendar window
            ...
            for (var d = start; d < end; d = d.AddDays(1))
            {
                if (d.DayOfWeek is DayOfWeek.Tuesday or DayOfWeek.Wednesday or DayOfWeek.Thursday)
                {
                    foreach (var slotKey in new[] { "Morning", "Afternoon" })
                    {
                        ...
                        // precedence: past → approved → pending → currentWeek → blockedDates → free
                        if (isPast)
                        {
                            status = "blocked";
                            reason = "past";
                        ...
                        else if (respectCurrentWeek && isCurrentWeek)
                        {
                            status = "blocked";
                            reason = "currentWeek";
                        }
...
#pragma warning restore CS0618
            ...
            // Build blocked set (for validation) – only care about the single date d
            var blockedPairs = new HashSet<(DateOnly date, string slotKey)>(new DateSlotComparer());
            if (settings != null)
            {
                var bl = settings.Value<BlockListModel>("blockedDates");
                if (bl != null)
                {
                    foreach (var item in bl)
                    {
                        ...
                        // if booking date d is not in this interval, skip
                        if (d < from || d > to)
                            continue;
                        ...
                        if (sel == "Morning") blockedPairs.Add((d, "Morning"));
                        else if (sel == "Afternoon") blockedPairs.Add((d, "Afternoon"));
                        else if (sel == "Both")
                        {
                            blockedPairs.Add((d, "Morning"));
                            blockedPairs.Add((d, "Afternoon"));
                        }
                    }
                }
            }
...
#pragma warning restore CS0618
                ...
                // IMPORTANT: don't set From → use configured SMTP 'From'
...
#pragma warning restore CS0618
        ...
        // ICS feed (Approved only, timed, title "Vitjan - Seinnapart - 13.30"):
        // /umbraco/api/bookingapi/feed.ics
        [HttpGet("feed.ics")]
        [AllowAnonymous]
...
#pragma warning restore CS0618
            ...
            foreach (var b in bookings)
            {
                ...
                // Build timed event: start = slotDate + arrivalTime, end = start + 2h
                DateTime? startDt = null;
                if (!string.IsNullOrWhiteSpace(time) &&
                    TimeSpan.TryParse(time, CultureInfo.InvariantCulture, out var ts))
                {
                    startDt = new DateTime(dt.Value.Year, dt.Value.Month, dt.Value.Day, ts.Hours, ts.Minutes, 0);
                }

                // Title pattern: "Vitjan - Seinnapart - 13.30"
                string friendlyTime = "";
                if (!string.IsNullOrWhiteSpace(time))
                    friendlyTime = time.Replace(":", ".");

                var summary = string.IsNullOrWhiteSpace(friendlyTime)
                    ? $"Vitjan - {label}"
                    : $"Vitjan - {label} - {friendlyTime}";

                // Backoffice URL (requires login)
                var backendUrl = $"https://www.kob.fo/umbraco#/content/content/edit/{b.Id}";

                // ICS-safe description text with link
                var description = $"Sí bóking í Umbraco: {backendUrl}";
                ...
                if (startDt.HasValue)
                {
                    var endDt = startDt.Value.AddHours(2);
                    var dtStartStr = startDt.Value.ToString("yyyyMMdd'T'HHmmss");
                    var dtEndStr   = endDt.ToString("yyyyMMdd'T'HHmmss");
                    sb.AppendLine($"DTSTART:{dtStartStr}");
                    sb.AppendLine($"DTEND:{dtEndStr}");
                }
                else
                {
                    // fallback: all-day event
                    var startDate = dt.Value.ToString("yyyyMMdd");
                    sb.AppendLine($"DTSTART;VALUE=DATE:{startDate}");
                }

                sb.AppendLine($"SUMMARY:{summary}");
                sb.AppendLine($"DESCRIPTION:{description}");
                sb.AppendLine($"URL:{backendUrl}");
        ...
        // ICS feed (Approved only, NO personal info, timed, same title pattern):
        // /umbraco/api/bookingapi/feed-public
        // /umbraco/api/bookingapi/feed-public.ics
        [HttpGet("feed-public")]
        [HttpGet("feed-public.ics")]
        [AllowAnonymous]
        [Produces("text/calendar")]
        public IActionResult IcsPublicFeed()
        {
            using var cref = _ctxFactory.EnsureUmbracoContext();
            var umb = cref.UmbracoContext;
...
#pragma warning restore CS0618
            ...
            sb.AppendLine("BEGIN:VCALENDAR");
            sb.AppendLine("VERSION:2.0");
            sb.AppendLine("PRODID:-//kob.fo//Booking Public Feed//FO");
            sb.AppendLine("CALSCALE:GREGORIAN");
            sb.AppendLine("METHOD:PUBLISH");
            ...
            foreach (var b in bookings)
            {
                ...
                // Text inside calendar booking (no direct backoffice link here if you want it "public")
                var description = "Sí bóking í Umbraco";
                description = description
                    .Replace("\\", "\\\\")
                    .Replace("\n", "\\n")
                    .Replace("\r", "");
                ...
                sb.AppendLine("BEGIN:VEVENT");
                sb.AppendLine($"UID:public-{b.Id}@kob.fo");
                sb.AppendLine($"DTSTAMP:{DateTime.UtcNow:yyyyMMdd'T'HHmmss'Z'}");
            ...
            var bytes = Encoding.UTF8.GetBytes(sb.ToString());
            return File(bytes, "text/calendar; charset=utf-8", "bookings-public.ics");
        }
    }
}
