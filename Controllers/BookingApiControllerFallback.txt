...
namespace Kob.Controllers
{
    ...
    public class BookingApiController : ControllerBase
    {
        ...
        // bookingSettings by GUID (contains approvalEmail)
...
#pragma warning restore CS0618
        ...
        [HttpGet("ping")]
        [AllowAnonymous]
        public IActionResult Ping() => Ok(new { ok = true, msg = "booking api alive", version = "v2.7-email-fofix-toString" });
...
#pragma warning restore CS0618
            ...
            if (settings != null)
            {
                ...
                if (bl != null)
                {
                    ...
                    {
                        var dt = item?.Content?.Value<DateTime?>("date");
                        if (dt == null) continue;
                        ...
                        if (debug)
                        {
                            debugInfo.Add(new
                            {
                                date = dOnly.ToString("yyyy-MM-dd"),
                                raw = extractedValue ?? "NULL",
                                normalized = sel ?? "NULL"
                            });
                        }

                        if (sel == "Morning") blockedPairs.Add((dOnly, "Morning"));
                        else if (sel == "Afternoon") blockedPairs.Add((dOnly, "Afternoon"));
                        else if (sel == "Both")
                        {
                            blockedPairs.Add((dOnly, "Morning"));
                            blockedPairs.Add((dOnly, "Afternoon"));
                        }
                    }
                }
            }
...
#pragma warning restore CS0618
            ...
            // Build blocked set (for validation)
            var blockedPairs = new HashSet<(DateOnly date, string slotKey)>(new DateSlotComparer());
            if (settings != null)
            {
                var bl = settings.Value<BlockListModel>("blockedDates");
            ...
            // Create content
            var nodeName = $"[Pending] {d:yyyy-MM-dd} – {ToFaroeseLabel(slotKey)} – {(string.IsNullOrWhiteSpace(dto.NameValue) ? "Unknown" : dto.NameValue)}";
...
#pragma warning restore CS0618
                ...
                var subject = $"Nýggj bílegging – {d:yyyy-MM-dd} {ToFaroeseLabel(slotKey)}";
                ...
                // EmailMessage expects a single string for "to"
...
#pragma warning restore CS0618
            ...
            foreach (var b in bookings)
            {
                ...
                var summary = $"{label} – {name} ({attendees} fólk)";
